...
from temporal_transforms import LoopPadding, TemporalRandomCrop, OrderedCrop
...
from arguments import parse_opts
import test
from optmz import ScheduledOptim
...
if __name__ == '__main__':
    ...
    if not opt.no_train:
        ...
        if opt.optimizer == 'adam':
            optimizer = optim.Adam(parameters, lr=0.1, betas=(0.9, 0.98), eps=1e-09, weight_decay=0, amsgrad=False)
        elif opt.optimizer == 'noam':
            optimizer = ScheduledOptim(
                optim.Adam(
                   filter(lambda x: x.requires_grad, parameters),
                   betas=(0.9, 0.98), eps=1e-09),
                   opt.feature_size, opt.n_warmup_steps)
        """"
                optimizer = optim.SGD(
                    parameters,
